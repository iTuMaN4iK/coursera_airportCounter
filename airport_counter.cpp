...
public:
  ...
  AirportCounter() : airportCount(){};

  // конструктор от диапазона элементов типа TAirport
  template <typename TIterator>
  AirportCounter(TIterator begin, TIterator end) : airportCount() {
    for (TIterator it = begin; it < end; it++) {
      ++airportCount[static_cast<size_t>(*it)];
    }
  };

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
    return airportCount[static_cast<size_t>(airport)];
  };

  // добавить данный элемент
  void Insert(TAirport airport) {
    ++airportCount[static_cast<size_t>(airport)];
  }
  ...
  void EraseAll(TAirport airport) { airportCount.fill(0); }
  ...
  Items GetItems() const {
    ...
    for (size_t i = 0; i < airportCount.size(); ++i) {
      Item p = {static_cast<TAirport>(i), airportCount[i]};
      items[i] = p;
    }
    return items;
  }
...
private:
  std::array<size_t, static_cast<uint32_t>(TAirport::Last_)> airportCount;
};
